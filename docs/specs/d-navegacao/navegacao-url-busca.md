...
- **RF11:** Barra de progresso global de navegacao (`NavigationProgress`) com feedback visual durante transicoes entre paginas
- **RF12:** Scroll spy (`useScrollSpy`) para destacar automaticamente o item de navegacao lateral correspondente a secao visivel no viewport
...
function useRecentSearches(maxItems = 5) {
  ...
  const addSearch = (query: string) => {
    setSearches((prev) =>
      [query, ...prev.filter((s) => s !== query)].slice(0, maxItems),
    );
  };
...
│   ├── use-scroll-spy.ts           # Scroll spy para navegacao por secoes
│   ├── use-debounce.ts
│   ├── use-local-storage.ts
│   └── use-recent-searches.ts
├── components/
│   ├── shared/
│   │   └── navigation-progress.tsx  # Barra de progresso global de navegacao
│   ├── navigation/
│   │   ├── breadcrumbs.tsx
│   │   └── command-palette.tsx
│   └── search/
...
**Componente:** barra fina (2px) no topo da pagina, cor `bg-primary`, animacao suave de largura.

```tsx
// Integrar no layout global (ja adicionado em src/app/[locale]/layout.tsx)
<NavigationProgress />
```

**Comportamento automatico:**

- Detecta cliques em `<a>` internos via event listener em captura
- Inicia a barra em 15%, cresce progressivamente ate ~90% durante carregamento
- Completa a 100% quando `usePathname()` muda (navegacao concluida)
- Ignora: links externos, `#hash`, `mailto:`, `tel:`, Ctrl/Cmd+click, `target="_blank"`, link para mesma pagina
- Safety net: auto-completa apos 8s para evitar barra travada

**Navegacao programatica:** usar as funcoes exportadas manualmente:

```tsx
import { startNavigation } from "@/components/shared";

startNavigation(); // Inicia a barra antes de router.push()
router.push("/nova-pagina"); // completeNavigation() e chamado automaticamente via usePathname
```

**Implementacao:** `useSyncExternalStore` com store externo module-level (compativel com React Compiler).
...
const sections = ["basic", "form", "data", "navigation"] as const;
const activeSection = useScrollSpy(sections);
// activeSection = "form" (se a secao #form estiver no topo do viewport)
```

**Parametros:**

- `sectionIds: readonly string[]` — IDs dos elementos `<section>` ou `<h2>` a observar
- `rootMargin?: string` — margem do IntersectionObserver (padrao: `"-10% 0px -70% 0px"`, destaca secao nos 30% superiores do viewport)

**Retorno:** `string | null` — ID da secao atualmente visivel, ou `null` se nenhuma.

**Implementacao:** `useSyncExternalStore` + `IntersectionObserver` com store externo por grupo de IDs. Prioriza a primeira secao visivel na ordem original dos IDs.

**Demonstracao:** pagina `/examples/components` com navegacao lateral e indicador animado (`motion.div` com `layoutId`).
...
- [x] RF11: NavigationProgress com barra de progresso global durante navegacao
- [x] RF12: useScrollSpy com indicador de secao ativa em navegacao lateral
